...
public class VOAActivity extends AppCompatActivity implements MediaPlayer.OnPreparedListener
            ...
            public void onClick(View v) {
                if (mNeedToFetchFromInternet) {
                    if (NetWorkUtils.getNetworkTypeName(mContext).equals(NetWorkUtils.NETWORK_TYPE_DISCONNECT)) {
                        Toast.makeText(VOAActivity.this, R.string.bad_internet, Toast.LENGTH_SHORT).show();
                        return;
                    }
    ...
    private void fetchVOAAudioFromInternet(String date, final String tag) {
        ...
        String url = "https://leancloud.cn:443/1.1/classes/VOA?where={\"postTime\":{\"$lt\":{\"__type\":\"Date\",\"iso\":\"" + date + "\"}},\"tag\":\"" + StringUtils.encodeText(tag) + "\",}}&limit=5&&order=-createdAt";
